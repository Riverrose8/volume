...
# Sigma URL template - ОТКЛЮЧЕНО
# Убрано по запросу пользователя из-за проблем с дубликатами и нестабильностью
SIGMA_URL_TEMPLATE = ""
...
async def fetch_bscscan_token_holders(session: aiohttp.ClientSession, token_address: str) -> Optional[int]:
    """
    Получает количество держателей токена через BaseScan API (для Base сети)
    Fallback источник когда DexTools недоступен
    """
    if not BSCSCAN_API_KEY:
        logger.debug("BaseScan API key not configured")
        return None
    
    try:
        # BaseScan API для получения информации о токене (использует тот же формат что и BSCScan)
        ...
        async with session.get(url, params=params, timeout=15) as r:
            if r.status != 200:
                logger.debug(f"BaseScan API error: {r.status}")
                return None
            ...
            if data.get("status") == "1" and data.get("result"):
                ...
                if holders:
                    ...
                    logger.info(f"✅ BaseScan: {token_address[:8]}... holders={holders_count}")
                    return holders_count
            else:
                logger.debug(f"BaseScan API returned no data: {data.get('message', 'Unknown error')}")
                return None
                
    except Exception as e:
        logger.debug(f"BaseScan API exception: {e}")
        return None
...
class TokenTracker:
    ...
    def mark_alerted(self, token_address: str):
        ...
        was_already_alerted = addr in self.alerted
        self.alerted.add(addr)
        self.last_alert_at[addr] = datetime.now(timezone.utc)
        
        # ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ
        if was_already_alerted:
            logger.warning(f"⚠️ mark_alerted: Token {addr[:10]}... was ALREADY in alerted set! (duplicate mark attempt)")
        else:
            logger.info(f"✅ mark_alerted: Token {addr[:10]}... added to alerted set (total: {len(self.alerted)})")
        
        # Сохраняем в файл для персистентного dedup с блокировкой файла
        try:
            import fcntl
            ...
            with open(temp_file, "w", encoding="utf-8") as f:
                # Блокируем файл для записи (Unix only)
                try:
                    fcntl.flock(f.fileno(), fcntl.LOCK_EX)
                except (AttributeError, OSError):
                    # Windows или файловая система не поддерживает блокировку
                    pass
            ...
            # fcntl недоступен (Windows), используем обычную запись
            try:
                payload = {"alerted": {a: self.last_alert_at.get(a, datetime.now(timezone.utc)).isoformat() for a in self.alerted}}
                temp_file = CACHE_FILE + ".tmp"
                with open(temp_file, "w", encoding="utf-8") as f:
    ...
    def is_alerted(self, token_address: str) -> bool:
        ...
        ВАЖНО: Перечитывает файл перед проверкой, чтобы видеть изменения от других процессов
        """
        addr = token_address.lower()
        
        # Сначала проверяем локальный кеш (быстро)
        if addr in self.alerted:
            return True
        
        # Перечитываем файл, чтобы видеть изменения от других процессов
        try:
            if os.path.exists(CACHE_FILE):
                with open(CACHE_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    ...
                    for a in alerted_map.keys():
                        if a.lower() not in self.alerted:
                            self.alerted.add(a.lower())
        except Exception as e:
            logger.debug(f"Failed to reload cache in is_alerted: {e}")
        
        # Проверяем снова после перечитывания
        return addr in self.alerted
...
# Глобальный Lock для синхронизации отправки алертов (предотвращает race condition)
_alert_lock = asyncio.Lock()

async def tg_send_with_dedup(
    ...
    """Отправка сообщения с дедупликацией: 1 адрес = 1 сигнал
    
    Использует asyncio.Lock для атомарной проверки и пометки токена,
    предотвращая race condition при параллельной обработке.
    """
    addr_lower = token_address.lower()
    
    # ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ: отслеживаем все попытки отправки
    import traceback
    caller = ''.join(traceback.format_stack()[-2:-1]).strip().split('\n')[-1] if len(traceback.format_stack()) > 1 else "unknown"
    logger.info(f"🔍 tg_send_with_dedup CALLED for {addr_lower[:10]}... from: {caller[:80]}")
    
    # КРИТИЧНО: Используем Lock для АТОМАРНОЙ операции проверки и пометки
    # ВСЕ операции (перечитывание файла, проверка, пометка) должны быть ВНУТРИ Lock
    # Это гарантирует, что только ОДИН вызов сможет пройти проверку и пометить токен
    async with _alert_lock:
        # Перечитываем файл ВНУТРИ Lock, чтобы видеть последние изменения
        try:
            if os.path.exists(CACHE_FILE):
                with open(CACHE_FILE, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    alerted_map = data.get("alerted", {}) if isinstance(data, dict) else {}
                    # Обновляем локальный кеш tracker СИНХРОННО
                    for a in alerted_map.keys():
                        a_lower = a.lower()
                        if a_lower not in tracker.alerted:
                            tracker.alerted.add(a_lower)
                            # Также обновляем last_alert_at для консистентности
                            try:
                                ts_str = alerted_map[a]
                                tracker.last_alert_at[a_lower] = datetime.fromisoformat(ts_str.replace('Z', '+00:00'))
                            except:
                                pass
                            logger.debug(f"🔄 Synced from file: {a_lower[:10]}...")
        except Exception as e:
            logger.debug(f"Failed to reload cache in lock: {e}")
        
        # АТОМАРНАЯ ПРОВЕРКА: проверяем ПОСЛЕ синхронизации с файлом
        # Используем прямое обращение к tracker.alerted, а не метод is_alerted (который может перечитывать файл снова)
        if addr_lower in tracker.alerted:
            logger.warning(f"⏳ BLOCKED: Token {addr_lower[:10]}... already in alerted set (total: {len(tracker.alerted)}) - skipping (caller: {caller[:50]})")
            return False
        
        # АТОМАРНАЯ ПОМЕТКА: помечаем ДО отправки, чтобы другие корутины сразу видели пометку
        # Добавляем в set СРАЗУ, чтобы другие корутины, ожидающие Lock, увидели пометку
        tracker.alerted.add(addr_lower)
        tracker.last_alert_at[addr_lower] = datetime.now(timezone.utc)
        logger.info(f"🔒 LOCKED: Token {addr_lower[:10]}... added to alerted set (total: {len(tracker.alerted)}) BEFORE sending")
        
        # Сохраняем в файл СРАЗУ, чтобы другие процессы видели пометку
        try:
            import fcntl
            ...
            try:
                ...
                logger.error(f"Failed to save cache in lock: {e}")
        except Exception as e:
            logger.error(f"Failed to save cache in lock: {e}")
    
    # Отправляем сообщение ВНЕ Lock, чтобы не блокировать другие операции
    success = await tg_send(session, text, chat_id=chat_id, reply_markup=reply_markup, disable_web_page_preview=disable_web_page_preview)
    
    if success:
        logger.info(f"✅ Alert sent for {token_address}")
    else:
        # Если отправка не удалась, оставляем пометку, чтобы не спамить при повторных ошибках
        logger.error(f"❌ Failed to send alert for {token_address} (but token remains marked)")
...
def get_target_channel_for_token(token_data: Dict) -> str:
    """Определяет канал для отправки алерта на основе источника токена"""
    source = token_data.get('source', '')
    
    # USDC и Virtuals пары идут в тестовый канал
    if source in ['dexscreener_usdc', 'dexscreener_virtuals']:
        return TELEGRAM_TEST_CHAT_ID if TELEGRAM_TEST_CHAT_ID else TELEGRAM_CHAT_ID
    
    # Все остальные токены (включая WETH пары) идут в основной канал
...
def build_trade_bot_keyboard(token_addr: str) -> Optional[Dict[str, Any]]:
    """Builds inline keyboard with Maestro / Bloom / Sigma / Based links for TEST_MODE.
    Uses env templates for Maestro/Bloom/Sigma if provided. Always includes Based link.
    Sigma button is ALWAYS included if SIGMA_URL_TEMPLATE is set (has default value).
    """
    try:
        ...
        # Sigma button - ОТКЛЮЧЕНО по запросу пользователя
        # if SIGMA_URL_TEMPLATE:
        #     try:
        #         sigma_url = SIGMA_URL_TEMPLATE.format(token=token_addr)
        #         row.append({
        #             "text": "Sigma",
        #             "url": sigma_url
        #         })
        #         logger.info(f"✅ Sigma button added for {token_addr[:10]}... (URL: {sigma_url[:60]}...)")
        #     except Exception as e:
        #         logger.error(f"❌ Error formatting Sigma URL for {token_addr[:10]}...: {e}")

        # Based bot link - fixed template (всегда добавляем)
        based_url = f"https://t.me/based_eth_bot?start=r_darkzodchi_b_{token_addr}"
        row.append({"text": "Based Bot", "url": based_url})

        if row:
            buttons.append(row)
            button_names = [btn.get("text", "?") for btn in row]
            logger.info(f"✅ Keyboard built with {len(row)} buttons for {token_addr[:10]}...: {', '.join(button_names)}")
            return {"inline_keyboard": buttons}
        else:
            logger.warning(f"⚠️ No buttons added for {token_addr[:10]}...")
    except Exception as e:
        logger.error(f"❌ Error building keyboard for {token_addr[:10]}...: {e}", exc_info=True)
    return None
        ...
        if "data" in info_data:
            ...
            if not result["holders_count"]:
                ...
                if bscscan_holders:
                    ...
                    logger.info(f"🔄 Using BaseScan fallback for holders: {bscscan_holders}")
                else:
...
async def scan_once(session: aiohttp.ClientSession, tracker: TokenTracker):
    ...
    try:
        ...
        # КРИТИЧНО: Нормализуем адреса токенов и дедуплицируем ДО обработки
        # Это предотвращает обработку одного токена дважды из разных источников
        normalized_tokens = {}
        for token_address, token_data in tokens.items():
            addr_lower = token_address.lower()
            # Если токен уже есть, берем тот, у которого больше объем
            if addr_lower not in normalized_tokens or token_data.get("volume_5m", 0) > normalized_tokens[addr_lower].get("volume_5m", 0):
                normalized_tokens[addr_lower] = token_data
        
        logger.info(f"📊 После дедупликации: {len(normalized_tokens)} уникальных токенов (было {len(tokens)})")
        
        # Логируем топ-5 токенов по объему для отладки
        if normalized_tokens:
            sorted_tokens = sorted(normalized_tokens.values(), key=lambda t: t["volume_5m"], reverse=True)[:5]
        ...
        # Обновляем tracker
        for token_address, token_data in normalized_tokens.items():
            # КРИТИЧНО: Атомарная проверка с Lock в начале цикла
            async with _alert_lock:
                # Перечитываем файл и синхронизируем данные
                try:
                    if os.path.exists(CACHE_FILE):
                        with open(CACHE_FILE, "r", encoding="utf-8") as f:
                            data = json.load(f)
                ...
                # Проверяем еще раз внутри Lock
                if token_address.lower() in tracker.alerted:
            ...
            if not is_new_token:
                ...
                if tracker.check_volume_spike(token_address, token_data["volume_5m"]):
                    # ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: если токен уже был отправлен, пропускаем
                    if tracker.is_alerted(token_address):
                        logger.info(f"⏳ SKIP: {token_data['token_symbol']} already alerted (volume spike path)")
                        continue
                    ...
                    try:
                        ...
                        if dextools_data:
                            ...
                            # Когда DexTools недоступен, устанавливаем holders_unavailable=True
                    ...
                    if keyboard:
                        buttons_count = len(keyboard.get("inline_keyboard", [{}])[0]) if keyboard.get("inline_keyboard") else 0
                        logger.info(f"📱 Keyboard created for {token_data['token_symbol']}: {buttons_count} buttons")
                    else:
                        logger.warning(f"⚠️ No keyboard created for {token_data['token_symbol']}!")
                    
                    # Отправляем алерт с дедупликацией
                    success = await tg_send_with_dedup(session, tracker, token_address, alert_text, reply_markup=keyboard, disable_web_page_preview=True)
                    if success:
                        ...
                        # При неудаче тоже выходим, чтобы не пытаться отправить через другие пути
                        continue
                
                logger.info(f"🔥 INSTANT ALERT: {token_data['token_symbol']} (established token with ${token_data['volume_5m']:,.0f} volume spike!)")
                
                # Уже помечено как alerted выше, если был volume spike
                ...
                try:
                    ...
                    # ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: если токен уже был отправлен, пропускаем
                    if tracker.is_alerted(token_address):
                        logger.info(f"⏳ SKIP: {token_data['token_symbol']} already alerted (established path)")
                        continue
                    
                    # Отправляем alert с дедупликацией (помечает как alerted автоматически)
                    message = format_alert(token_data, checks_count=0, is_established=True)
                    # Используем новую логику выбора канала
                    target_chat = get_target_channel_for_token(token_data)
                    # ВСЕГДА создаем клавиатуру с кнопками (включая Sigma)
                    keyboard = build_trade_bot_keyboard(token_address)
                    if keyboard:
                        buttons_count = len(keyboard.get("inline_keyboard", [{}])[0]) if keyboard.get("inline_keyboard") else 0
                        logger.info(f"📱 Keyboard for {token_data['token_symbol']} (established): {buttons_count} buttons")
                    success = await tg_send_with_dedup(session, tracker, token_address, message, chat_id=target_chat, reply_markup=keyboard)
            ...
            if token_data.get("is_new_token", True) and token_data.get("volume_5m", 0) >= MIN_VOLUME_SKIP_HOLDERS:
                ...
                # Пометка is_alerted теперь внутри tg_send_with_dedup
                # Получаем данные о holders и security из DexTools
                try:
                    dextools_data = await fetch_dextools_token_data(session, token_address)
                    if dextools_data:
                ...
                # ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: если токен уже был отправлен, пропускаем
                if tracker.is_alerted(token_address):
                    logger.info(f"⏳ SKIP: {token_data['token_symbol']} already alerted (new high-volume path)")
                    continue

                # Отправляем alert с дедупликацией
                ...
                if keyboard:
                    buttons_count = len(keyboard.get("inline_keyboard", [{}])[0]) if keyboard.get("inline_keyboard") else 0
                    logger.info(f"📱 Keyboard for {token_data['token_symbol']} (new high-volume): {buttons_count} buttons")
                success = await tg_send_with_dedup(session, tracker, token_address, message, chat_id=target_chat, reply_markup=keyboard)
                if success:
            ...
            if tracker.check_volume_spike(token_address, token_data["volume_5m"]):
                ...
                if keyboard:
                    buttons_count = len(keyboard.get("inline_keyboard", [{}])[0]) if keyboard.get("inline_keyboard") else 0
                    logger.info(f"📱 Keyboard for {token_data['token_symbol']} (volume spike): {buttons_count} buttons")
                
                # Отправляем алерт с дедупликацией
                success = await tg_send_with_dedup(session, tracker, token_address, alert_text, chat_id=target_chat, reply_markup=keyboard, disable_web_page_preview=True)
            ...
            # Проверяем стабильность только если токен еще не был отправлен
            if not tracker.is_alerted(token_address) and tracker.is_stable(token_address):
                ...
                # ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: если токен уже был отправлен, пропускаем
                if tracker.is_alerted(token_address):
                    logger.info(f"⏳ SKIP: {token_data['token_symbol']} already alerted (stable path)")
                    continue
                ...
                # Используем новую логику выбора канала
                target_chat = get_target_channel_for_token(token_data)
                # КРИТИЧНО: Дополнительная атомарная проверка ПЕРЕД отправкой
                # Это предотвращает race condition между проверкой в начале цикла и вызовом tg_send_with_dedup
                async with _alert_lock:
                    # Перечитываем файл и синхронизируем данные
                    try:
                        if os.path.exists(CACHE_FILE):
                    ...
                    # Проверяем еще раз внутри Lock
                    if token_address.lower() in tracker.alerted:
                        logger.warning(f"⏳ DOUBLE-CHECK BLOCKED: {token_data['token_symbol']} already alerted before tg_send_with_dedup")
                        continue
                ...
                if keyboard:
                    buttons_count = len(keyboard.get("inline_keyboard", [{}])[0]) if keyboard.get("inline_keyboard") else 0
                    logger.info(f"📱 Keyboard for {token_data['token_symbol']} (stable): {buttons_count} buttons")
                success = await tg_send_with_dedup(session, tracker, token_address, message, chat_id=target_chat, reply_markup=keyboard)
...
if __name__ == "__main__":
    # Логируем конфигурацию кнопок при запуске
    logger.info(f"🔧 Configuration check:")
    logger.info(f"   MAESTRO_URL_TEMPLATE: {'✅ Set' if MAESTRO_URL_TEMPLATE else '❌ Empty'}")
    logger.info(f"   BLOOM_URL_TEMPLATE: {'✅ Set' if BLOOM_URL_TEMPLATE else '❌ Empty'}")
    logger.info(f"   SIGMA_URL_TEMPLATE: {'✅ Set' if SIGMA_URL_TEMPLATE else '❌ Empty'} ({SIGMA_URL_TEMPLATE[:60] if SIGMA_URL_TEMPLATE else 'N/A'}...)")
    
    # Проверка на множественные экземпляры
    import subprocess
    try:
        # Используем ps aux для более надежного поиска (работает с любым путем к Python)
        ...
        # Ищем все процессы с main_base.py
        for line in result.stdout.split('\n'):
            if 'main_base.py' in line and str(current_pid) not in line:
                # Извлекаем PID (второе поле в ps aux)
                parts = line.split()
                if len(parts) > 1:
                    pid = parts[1]
        ...
        if other_instances:
            logger.error(f"❌ ОБНАРУЖЕНО {len(other_instances)} других экземпляров Base бота (PIDs: {', '.join(other_instances)})")
            logger.error("❌ Остановите другие экземпляры перед запуском!")
            logger.error("   Команда: ./start_base_bot.sh (остановит старые и запустит новый)")
            exit(1)
        else:
            logger.info(f"✅ Проверка пройдена: других экземпляров не обнаружено (PID: {current_pid})")
    except Exception as e:
        logger.warning(f"⚠️ Не удалось проверить другие экземпляры: {e}")
        logger.warning("⚠️ Продолжаем запуск, но убедитесь, что не запущено других экземпляров")
