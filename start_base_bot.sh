#!/bin/bash
# Скрипт для безопасного запуска Base бота
...
echo "=========================================="
echo "🚀 Запуск Base бота"
echo "=========================================="

# Проверяем наличие других экземпляров (ищем main_base.py в команде, независимо от пути к Python)
EXISTING_PIDS=$(ps aux | grep -E "[P]ython.*main_base.py|[p]ython3.*main_base.py|main_base.py" | grep -v grep | awk '{print $2}' | tr '\n' ' ')
...
if [ -n "$EXISTING_PIDS" ]; then
    ...
    REMAINING=$(ps aux | grep -E "[P]ython.*main_base.py|[p]ython3.*main_base.py|main_base.py" | grep -v grep | awk '{print $2}' | tr '\n' ' ')
    if [ -n "$REMAINING" ]; then
...
echo ""
echo "🚀 Запускаю Base бот..."
echo ""

# Запускаем бот в фоне с перенаправлением вывода в лог
nohup python3 main_base.py >> new_tokens_bot.log 2>&1 &
...
if ps -p $BOT_PID > /dev/null 2>&1; then
    echo "✅ Base бот успешно запущен (PID: $BOT_PID)"
